...
/**
 * Горизонтальная панель фильтра для витрины закупки.
 *
 * Дерево открывается сразу на странице в `Popover` под кнопкой «Фильтр».
 * Без модальных слоёв — пользователь остаётся в контексте витрины.
 */
export function FilterBar({
    ...
    if (!hasTree) {
        return (
            <div className="sticky top-14 z-10 -mx-4 flex items-center justify-between gap-3 border-b border-border-soft bg-bg-base/95 px-4 py-2 backdrop-blur supports-[backdrop-filter]:bg-bg-base/75 sm:mx-0 sm:px-0">
    ...
    return (
        <div className="sticky top-14 z-10 -mx-4 flex flex-wrap items-center gap-2 border-b border-border-soft bg-bg-base/95 px-4 py-2 backdrop-blur supports-[backdrop-filter]:bg-bg-base/75 sm:mx-0 sm:px-0">
            <Popover>
