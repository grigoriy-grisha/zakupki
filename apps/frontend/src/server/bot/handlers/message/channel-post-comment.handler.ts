...
/**
 * ChannelPostShopCommentHandler — обработчик `is_automatic_forward` в обсуждении канала.
 * Индексирует `channelPostMessageId → discussionMessageId` в Redis.
 *
 * Shop-комментарий и статус-комментарии отправляет TgPostWorker (через очередь).
 */
export class ChannelPostShopCommentHandler implements MessageHandler {
    readonly filter = 'autoforward' as const;
